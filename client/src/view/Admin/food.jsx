...
function Food() {
  ...
  return (
    <div id="containerU">
      ...
      <div id="contentD">
        <h1 className='text-3xl font-bold mt-2 mb-4'>Food Assistance</h1>
        <table className='table-auto w-full'>
        <thead className='bg-red-800 text-white'>
            <tr>
              <th  className='font-normal py-1.5 px-2'>Name</th>
              <th  className='font-normal py-1.5 px-2'>Type(s) of Food</th>
          ...
          <tbody>
            {foodAssistance.map((request) => (
              <tr key={request._id} className='even:bg-gray-200'>
                ...
                <td className='px-10 py-2'>{request.typesOfFood}</td>
                ...
                <td className='px-10 py-2'>{request.numberOfPax}</td>
                <td className='px-10 py-2'>{request.approved ? 'Yes' : 'No'}</td>
                <td className='px-10 py-2'>
                {!request.approved ? (
                  <button type="button" className="enable-button" onClick={() => approveFoodRequest(request._id)}>
                    Approve
                  </button>
                ) : (
                  <span>Approved</span>
